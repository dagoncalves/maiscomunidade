...
class ElatedfCoreImport {
	...
	function eltd_update_meta_fields_after_import( $folder ) {
		global $wpdb;

		$url         = home_url( '/' );
		$demo_urls   = $this->eltd_import_get_demo_urls( $folder );

		foreach ( $demo_urls as $demo_url ) {
			$sql_query   = "SELECT meta_id, meta_value FROM wp_postmeta WHERE meta_key LIKE 'eltd%' AND meta_value LIKE '" . esc_url( $demo_url ) . "%';";
			$meta_values = $wpdb->get_results( $sql_query );

			if ( ! empty( $meta_values ) ) {
				foreach ( $meta_values as $meta_value ) {
					$new_value = $this->eltd_recalc_serialized_lengths( str_replace( $demo_url, $url, $meta_value->meta_value ) );
	...
	function eltd_update_options_after_import( $folder ) {
		$url       = home_url( '/' );
		$demo_urls = $this->eltd_import_get_demo_urls( $folder );

		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'eltd_options_findme' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( 'eltd_options_findme', $new_global_values );
		}
	}

	function eltd_import_get_demo_urls( $folder ) {
		$demo_urls  = array();
	...
	function eltd_recalc_serialized_lengths( $sObject ) {
		$ret = preg_replace_callback( '!s:(\d+):"(.*?)";!', 'eltd_recalc_serialized_lengths_callback', $sObject );

		return $ret;
	}

	function eltd_recalc_serialized_lengths_callback( $matches ) {
